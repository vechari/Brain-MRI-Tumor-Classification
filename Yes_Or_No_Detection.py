...
train_model(model, train_ds, val_ds, epochs=50) #train model for specified number of epochs


#Prediction on novel Image: Classify new image (not in dataset) as "no" tumor or "yes" tumor
pred_path = 'pred'
pred_dir = get_path(pred_path) #get path to prediction file

# extract image files
image_files = os.listdir(pred_dir)
img_nums = [0,1,2,3] #image number 1 is no tumer, image number 0 contains a tumor
for img_num in img_nums:
  ...
  gradcam = GradCAM(functional_model, outcome, 'conv2d_2') #create gradcam model
